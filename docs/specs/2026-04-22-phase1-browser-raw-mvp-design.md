...
3. `Decode & Preview`
   负责 `embedded preview -> quick preview -> HQ image` 三段式图像资产产出。
...
type RenderState = {
  ...
  lastRenderSource?: 'embedded' | 'quick' | 'hq'
...
- `canExport`：`hqImage.ready` 且当前风格合法且无进行中的导出任务且渲染状态未失败
...
2. `quick decode preview`
3. `HQ decode`

生成优先级：

`embedded first impression -> quick fallback -> HQ final asset`

显示优先级：

`hq > quick > embedded > none`

### 5.3 预览状态规则

- 如果 embedded preview ready，应尽快显示首个可见图像
- 如果 embedded preview 不可用或失败，应自动回退到 quick preview
- HQ decode 始终作为后台正式资产生成路径
- 快速预览 ready 后即可进入编辑态
- 只有 HQ ready 后才能进入可导出态
...
当 HQ ready 后，系统执行“输入资产升级”，但不重建编辑页。
...
- embedded fail：静默回退到 quick/hq
- quick fail：若 hq 仍在进行，继续等待
- HQ fail：保留已有预览，禁用导出
...
- 常见 24MP 级 RAW 的 HQ 就绪目标为约 5 秒级
- 高像素测试 RAW 的 HQ 就绪目标为约 10 秒级
- 风格切换应具备明显即时反馈
