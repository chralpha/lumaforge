...
`single RAW upload -> fast preview -> bounded HQ preview -> builtin/custom LUT -> compare -> JPEG export`
...
第一优先级是让用户尽快看到可风格化图像，并在 bounded HQ 结果完成后无感升级。
...
- bounded HQ：目标 5 到 8 秒，输出像素数受 preview policy 限制
...
embedded preview 只用于首屏显示，不作为 bounded HQ 或导出权威源，也不声明为 Linear ProPhoto。
...
- `maxOutputPixels`: 由 preview policy 提供，默认约 `8MP` 到 `12MP`
...
- 切换目标 Log 不触发重新解码
- compare 模式不触发重新解码
- bounded HQ 升级只替换输入 frame，不改变 active style
...
成功时直接返回 JPEG/bitmap bytes。失败时返回 `null`，preview-pipeline 自动等待 quick/bounded-HQ。
...
后台有界 decode，输出与 quick 相同协议但像素预算更高。它不得返回原始分辨率 preview frame。

bounded HQ ready 后执行 source upgrade，不重置：
...
- `low`：embedded + quick；bounded HQ 需要降级或静默跳过
- `normal`：embedded + quick + bounded HQ
- `high`：允许更高像素 bounded HQ 与导出压力测试

内存估算至少考虑：

- RAW file size
- decoded quick frame size
- decoded bounded HQ frame size
- wasm heap
- GPU texture copy
...
- quick fail 不影响 bounded HQ 编排策略，但不能阻塞在 full-resolution preview 等待上
- 错误码稳定
...
- `decodeBoundedHq` 输出同协议并遵守像素 cap
- quick 尺寸小于或等于 bounded HQ，bounded HQ 小于原始分辨率
...
- 新 runtime quick/bounded-HQ decode + `RGB16` 输出
...
- quick/bounded-HQ 均声明 `linear-prophoto-rgb`
- 默认 `noAutoBright: true`
- 同一张 RAW 的 quick/bounded-HQ 风格趋势一致
...
- Phase 1 全部自动测试仍通过
- Phase 1 手动测试矩阵仍通过
- 至少 3 个 RAW fixture 完成 embedded/quick/bounded-HQ 验证
- 24MP 级样张达到 quick 2 到 4 秒、bounded HQ 5 到 8 秒的稳妥预算，或有明确记录说明超预算原因
