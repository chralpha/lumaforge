...
The default fetch command downloads only `purpose: "ci-smoke"` fixtures.
It is
...
The JSON report is engineering evidence only.
Do not convert it into official
...
copy the JSONL output.
The benchmark page does not read fixture paths from the
environment.
...
missing.
Do not hard-code local absolute fixture paths in runtime source,
