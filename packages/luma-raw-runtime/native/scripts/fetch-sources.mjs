...
async function downloadArchive(source, archivePath) {
  ...
  try {
    ...
    const timeout = error instanceof Error && (error.name === 'AbortError' || error.name === 'TimeoutError')
      ...
      : '';
    throw new Error(`Failed to download ${formatSource(source)} from ${source.url}${timeout}: ${details}`);
  }
}
...
function isUnsafeArchiveEntry(entry) {
  ...
  return path.posix.isAbsolute(normalizedEntry)
...
async function listArchiveEntries(source, archivePath) {
  return new Promise((resolve, reject) => {
    ...
    tar.on('close', (code) => {
      ...
      reject(new Error(`tar -tzf failed for ${formatSource(source)} with exit code ${code}${details}`));
    });
  });
}
...
async function validateArchiveEntries(source, archivePath) {
  ...
  if (unsafeEntry) {
    throw new Error(`Unsafe archive entry for ${formatSource(source)}: ${unsafeEntry}`);
  }
}
...
async function extractArchive(source, archivePath, destinationDir) {
  await new Promise((resolve, reject) => {
    ...
    tar.on('close', (code) => {
      ...
      reject(new Error(`tar -xzf failed for ${formatSource(source)} with exit code ${code}`));
    });
  });
...
function validateBasenameOnlyField(source, index, field) {
  ...
  if (
    ...
    throw new TypeError(`Invalid native source lockfile: sources[${index}].${field} must be a basename-only relative name`);
  }
}
...
function validateSource(source, index) {
  ...
  for (const field of sourceFields) {
    if (typeof source[field] !== 'string') {
      throw new TypeError(`Invalid native source lockfile: sources[${index}].${field} must be a string`);
    }
  }

  if (!sha256Pattern.test(source.sha256)) {
    throw new TypeError(`Invalid native source lockfile: sources[${index}].sha256 must be 64 lowercase hex characters`);
  }
  ...
  try {
    ...
    throw new TypeError(`Invalid native source lockfile: sources[${index}].url must be a valid URL`);
  }

  if (url.protocol !== 'http:' && url.protocol !== 'https:') {
    throw new TypeError(`Invalid native source lockfile: sources[${index}].url must use http or https`);
  }
