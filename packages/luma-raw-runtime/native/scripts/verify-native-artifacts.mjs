...
const jsArtifact = {
  file: 'luma_raw.js',
  path: path.join(distNativeDir, 'luma_raw.js'),
};
const wasmArtifact = {
  file: 'luma_raw.wasm',
  path: path.join(distNativeDir, 'luma_raw.wasm'),
};
const writeProvenance = process.argv.includes('--write-provenance');
const unknownArgs = process.argv.slice(2).filter((arg) => arg !== '--write-provenance');
...
function validateSourceLock(sourceLock) {
  ...
  if (sourceLock.schemaVersion !== 1) {
    throw new TypeError('Invalid native source lockfile: schemaVersion must be 1');
  }

  if (!isObject(sourceLock.toolchain)) {
    throw new TypeError('Invalid native source lockfile: toolchain must be an object');
  }

  if (!Array.isArray(sourceLock.sources) || sourceLock.sources.length === 0) {
    throw new TypeError('Invalid native source lockfile: sources must be a non-empty array');
  }

  sourceLock.sources.forEach((source, index) => {
    if (!isObject(source)) {
      throw new TypeError(`Invalid native source lockfile: sources[${index}] must be an object`);
    }

    for (const field of ['name', 'version', 'url', 'sha256']) {
      if (typeof source[field] !== 'string' || source[field].length === 0) {
        throw new TypeError(`Invalid native source lockfile: sources[${index}].${field} must be a non-empty string`);
      }
    }
  });
}
...
function validateProvenance(provenance) {
  ...
  if (provenance.schemaVersion !== 1) {
    throw new TypeError('Invalid native artifact provenance: schemaVersion must be 1');
  }

  if (!isObject(provenance.artifacts)) {
    throw new TypeError('Invalid native artifact provenance: artifacts must be an object');
  }

  for (const key of ['js', 'wasm']) {
    if (!isObject(provenance.artifacts[key])) {
      throw new TypeError(`Invalid native artifact provenance: artifacts.${key} must be an object`);
    }
  }
}
...
function verifyField(provenance, fieldPath, current) {
  const recorded = fieldPath.split('.').reduce((value, field) => value?.[field], provenance);
...
async function scanGeneratedOutput(artifact, { required = true } = {}) {
  ...
  try {
    ...
    if (required) {
      const details = error instanceof Error ? error.message : String(error);
      throw new Error(`Failed to scan ${artifact.file} for forbidden native baseline markers: ${details}`);
    }
  ...
  return forbiddenGeneratedMarkers
    ...
    .map((marker) => `${artifact.file} contains forbidden native baseline marker: ${marker}`);
}

async function verifyGeneratedOutputs() {
  const matches = [
    ...await scanGeneratedOutput(jsArtifact),
    ...await scanGeneratedOutput({ file: 'provenance.json', path: provenancePath }),
    ...await scanGeneratedOutput(wasmArtifact, { required: false }),
  ];

  if (matches.length > 0) {
    throw new Error(`Forbidden native baseline markers found in generated artifacts:\n${matches.join('\n')}`);
  }
}
...
async function buildProvenance() {
  ...
  const sourceLock = await readJsonFile(sourceLockPath, 'native source lockfile');
...
async function main() {
  ...
  if (writeProvenance) {
    await fs.writeFile(provenancePath, `${JSON.stringify(currentProvenance, null, 2)}\n`);
  ...
  const recordedProvenance = await readJsonFile(provenancePath, 'native artifact provenance');
  validateProvenance(recordedProvenance);

  verifyField(recordedProvenance, 'sourceLockSha256', currentProvenance.sourceLockSha256);
  ...
  verifyField(recordedProvenance, 'artifacts.js.file', currentProvenance.artifacts.js.file);
  verifyField(recordedProvenance, 'artifacts.js.sha256', currentProvenance.artifacts.js.sha256);
  verifyField(recordedProvenance, 'artifacts.wasm.file', currentProvenance.artifacts.wasm.file);
  verifyField(recordedProvenance, 'artifacts.wasm.sha256', currentProvenance.artifacts.wasm.sha256);
  await verifyGeneratedOutputs();
