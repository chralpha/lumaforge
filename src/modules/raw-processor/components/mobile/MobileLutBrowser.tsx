    ...
    <AnimatePresence>
      {props.open && (
        <m.aside
          ...
          className="absolute inset-x-0 bottom-0 z-[46] grid max-h-[82%] grid-rows-[auto_minmax(0,1fr)] rounded-t-2xl border-t border-border bg-material-opaque pb-safe-offset-3 text-text shadow-2xl"
        ...
        >
          ...
          <div className="grid min-h-0 gap-[18px] overflow-y-auto px-4 pb-5 pt-1">
            <section className="grid gap-2.5">
              ...
              <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 rounded-xl border border-border/60 bg-fill-secondary/50 p-3">
                <span className="min-w-0 truncate text-sm font-semibold text-text">
                  {props.currentLutName ?? '—'}
                </span>
                <button
                  type="button"
                  className="rounded-md border border-border bg-background px-2.5 py-1.5 text-xs font-semibold text-text transition-colors hover:bg-fill-secondary disabled:cursor-not-allowed disabled:opacity-50"
            ...
            <section className="grid gap-2.5">
              ...
              <Dropzone
                ...
                className="grid min-h-20 place-items-center border-border/70 bg-fill-secondary/50 px-3 py-4 text-center"
                ...
                >
                  ...
                  {props.onlineLutSources.state.resources.map((resource) => {
                    ...
                    return (
                      <div
                        key={resource.id}
                        className="grid gap-1.5 rounded-xl border border-border/60 bg-fill-secondary/50 p-2.5"
                      >
                        ...
                        <div className="grid gap-1.5">
                          {entries.map((entry) => (
                            <button
                              key={entry.id}
                              type="button"
                              className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] items-center gap-2 rounded-md border border-border bg-background px-2.5 py-2 text-left transition-colors hover:bg-fill-secondary disabled:cursor-not-allowed disabled:opacity-50"
                              disabled={props.disabled}
                ...
                <div className="grid gap-1.5">
                  {profileSuggestions.map((profile) => (
                    <button
                      key={profile.id}
                      type="button"
                      className="grid min-w-0 gap-1 rounded-md border border-border bg-fill-secondary/50 px-2.5 py-2 text-left transition-colors hover:bg-fill-secondary disabled:cursor-not-allowed disabled:opacity-50"
                      disabled={props.disabled}
